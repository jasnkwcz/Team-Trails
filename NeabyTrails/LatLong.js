document.addEventListener('DOMContentLoaded', actButton);
  ...
  document.getElementById("nearbyTrails").addEventListener("click", function(event) {
    ...
    req.addEventListener("load", function() {
      if (req.status >= 200 && req.status < 400) {
        ...
        document.getElementById("weatherLat").textContent = "Latitude " + response.coord.lat;
        document.getElementById("weatherLong").textContent = "Longitude " + response.coord.lon;
